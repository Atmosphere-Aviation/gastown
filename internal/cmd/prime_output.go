...
// This provides fallback startup instructions when no work is hooked.
// Hook status and mail are pre-injected by SessionStart hooks, so we don't
// instruct agents to run gt hook or gt mail inbox redundantly.
func outputStartupDirective(ctx RoleContext) {
	...
	case RoleMayor:
		...
		fmt.Println("---")
		fmt.Println()
		fmt.Println("**No work on hook.** Hook and mail status were injected above by SessionStart.")
		fmt.Println("If a 🤝 HANDOFF message is shown, read and act on it.")
		fmt.Println("Otherwise, await user instruction.")
	case RoleWitness:
		fmt.Println()
		fmt.Println("---")
		fmt.Println()
		fmt.Println("**No patrol on hook.** Hook and mail status were injected above by SessionStart.")
		fmt.Println("Create a new patrol: `" + cli.Name() + " patrol new`")
	case RolePolecat:
		fmt.Println()
		fmt.Println("---")
		fmt.Println()
		fmt.Println("**No work on hook.** Check if mail was injected above by SessionStart.")
		fmt.Println("If mail has work instructions → execute that work.")
		fmt.Println("If NO mail → run `" + cli.Name() + " done` IMMEDIATELY.")
		fmt.Println()
	...
	case RoleCrew:
		...
		fmt.Println()
		fmt.Println("**No work on hook.** Hook and mail status were injected above by SessionStart.")
		fmt.Println("If a 🤝 HANDOFF message is shown, read and continue the work.")
		fmt.Println("Otherwise, await user instruction.")
	case RoleDeacon:
		...
		fmt.Println()
		fmt.Println("**No patrol on hook.** Hook and mail status were injected above by SessionStart.")
		fmt.Println("Signal awake: `" + cli.Name() + " deacon heartbeat \"starting patrol\"`")
		fmt.Println("Create a new patrol: `bd mol wisp mol-deacon-patrol`")
	case RoleBoot:
		fmt.Println()
		fmt.Println("---")
		fmt.Println()
